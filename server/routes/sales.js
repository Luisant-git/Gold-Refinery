// server/routes/sales.js — PostgreSQL
...
router.get('/customer-ob/:customerId', async (req, res) => {
  try {
    const r = await getPool().query(`SELECT voucher_no, voucher_date, COALESCE(balance_amount,0) AS balance_amount FROM sales_vouchers WHERE customer_id=$1 ORDER BY created_at DESC LIMIT 10`, [req.params.customerId]);
...
router.get('/', async (req, res) => {
  try {
    ...
    const r = await getPool().query(`SELECT * FROM sales_vouchers ${where} ORDER BY created_at DESC`, params);
...
router.get('/:id', async (req, res) => {
  try {
    const pool=getPool();
    const v=await pool.query(`SELECT * FROM sales_vouchers WHERE id=$1`,[req.params.id]);
    const items=await pool.query(`SELECT * FROM sales_voucher_items WHERE voucher_id=$1 ORDER BY sno`,[req.params.id]);
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const seqR = await client.query(`UPDATE voucher_sequences SET current_no=current_no+1 WHERE voucher_type='SALES' RETURNING prefix, current_no`);
    ...
    const vRes = await client.query(`
      INSERT INTO sales_vouchers (voucher_no,voucher_date,customer_id,mobile,customer_name,total_gross_wt,total_pure_wt,gross_amount,deductions,net_amount,amount_paid,balance_amount,payment_mode,rate_per_gram,remarks,status,created_at,updated_at)
