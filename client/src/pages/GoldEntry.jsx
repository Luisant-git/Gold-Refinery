...
export default function GoldEntry() {
  ...
  const [form,setForm]=useState({entry_date:today,mobile:'',customer_name:'',customer_id:null,gold_wt:'',touch:'99.90',entry_type:'OUT',remarks:''});
  const [list,setList]=useState([]); const [msg,setMsg]=useState(null); const [saving,setSaving]=useState(false);

  useEffect(()=>{ load(); },[]);
  const load=()=>goldEntryAPI.getAll().then(setList).catch(()=>{});
  ...
  const totalIn  = list.filter(r=>r.entry_type==='IN').reduce((s,r)=>s+(parseFloat(r.pure_wt)||0),0);
  ...
  return (
    <div className="page">
      ...
      <div className="card" style={{marginTop:14}}>
        ...
        {goldWt>0&&<div style={{marginBottom:12,padding:'7px 12px',background:'#F0FAF4',border:'1px solid rgba(26,110,64,0.25)',borderRadius:6,fontSize:13}}>
          Pure Wt: <strong style={{...monoStyle,color:'var(--green)'}}>{goldWt.toFixed(3)}g × {touch}% ÷ 100 = {pureWt.toFixed(3)}g</strong>
        </div>}
        <button className="btn btn-primary" onClick={handleSave} disabled={saving}>{saving?'Saving...':'✦ Save Gold Entry'}</button>
      </div>
      ...
      <div className="card">
        ...
        {list.length===0?<div className="empty-state"><div className="empty-icon">⬡</div><p>No entries yet</p></div>:(
          <div className="table-container"><table>
            <thead><tr><th>Date</th><th>Entry No</th><th>Customer</th><th>Mobile</th><th className="right">Gold Wt (g)</th><th className="right">Touch (%)</th><th className="right">Pure Wt (g)</th><th className="center">Type</th></tr></thead>
            <tbody>{list.map((r,i)=>(
              <tr key={i}>
