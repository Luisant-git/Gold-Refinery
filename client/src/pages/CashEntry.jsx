...
function MobileAC({ value, onChange, onSelect }) {
  const [sugg, setSugg] = useState([]); const [open, setOpen] = useState(false); const ref = useRef(null);
  useEffect(()=>{ const h=e=>{if(ref.current&&!ref.current.contains(e.target))setOpen(false);}; document.addEventListener('mousedown',h); return()=>document.removeEventListener('mousedown',h); },[]);
  const hc = async e => { const v=e.target.value; onChange(v); if(v.length>=2){const r=await customerAPI.search(v).catch(()=>[]); setSugg(r); setOpen(r.length>0);} else{setSugg([]);setOpen(false);} };
  const pick = row => { onChange(row.mobile); onSelect(row); setOpen(false); };
  return (
    <div style={{position:'relative'}} ref={ref}>
      <input type="tel" value={value} onChange={hc} maxLength={15} style={{width:'100%'}} autoComplete="off" />
      {open&&<div style={{position:'absolute',top:'100%',left:0,right:0,zIndex:999,background:'#fff',border:'1.5px solid var(--gold-accent)',borderRadius:6,boxShadow:'0 4px 18px rgba(100,70,0,0.15)',maxHeight:200,overflowY:'auto'}}>
        {sugg.map(c=><div key={c.id} onClick={()=>pick(c)} style={{padding:'9px 13px',cursor:'pointer',borderBottom:'1px solid var(--border)',fontSize:14}} onMouseEnter={e=>e.currentTarget.style.background='#FBF7EE'} onMouseLeave={e=>e.currentTarget.style.background='#fff'}><strong style={{color:'var(--gold-dark)'}}>{c.mobile}</strong> — {c.name}</div>)}
      </div>}
    </div>
  );
}
...
export default function CashEntry() {
  ...
  const [form, setForm] = useState({ entry_date:today, mobile:'', customer_name:'', customer_id:null, amount:'', entry_type:'OUT', remarks:'' });
  ...
  const onSelect = c => setForm(f=>({...f,customer_name:c.name,customer_id:c.id}));
  ...
  const handleSave = async () => {
    ...
    if(!form.amount||parseFloat(form.amount)<=0){setMsg({type:'danger',text:'Amount required'});return;}
    ...
    try {
      ...
      const r=await cashEntryAPI.create({...form,customer_id:custId});
      setMsg({type:'success',text:`Cash Entry ${r.entry_no} saved!`});
      setForm({entry_date:today,mobile:'',customer_name:'',customer_id:null,amount:'',entry_type:'OUT',remarks:''});
      load();
    ...
    <div className="page">
      ...
      <div className="two-col" style={{marginBottom:0}}>
        <div className="stat-card" style={{borderColor:'rgba(26,110,64,0.3)'}}>
          ...
          <div className="stat-value" style={{color:'var(--green)',...monoStyle}}>₹{totalIn.toLocaleString('en-IN')}</div>
        ...
        <div className="stat-card" style={{borderColor:'rgba(184,50,50,0.3)'}}>
          ...
          <div className="stat-value" style={{color:'var(--red)',...monoStyle}}>₹{totalOut.toLocaleString('en-IN')}</div>
      ...
      <div className="card" style={{marginTop:14}}>
        ...
        <button className="btn btn-primary" onClick={handleSave} disabled={saving}>{saving?'Saving...':'✦ Save Cash Entry'}</button>
      </div>

      <div className="card">
        <div className="card-title">Recent Cash Entries</div>
        {list.length===0?<div className="empty-state"><div className="empty-icon">₹</div><p>No entries yet</p></div>:(
          <div className="table-container"><table>
            <thead><tr><th>Date</th><th>Entry No</th><th>Customer</th><th>Mobile</th><th className="right">Amount (₹)</th><th className="center">Type</th><th>Remarks</th></tr></thead>
            <tbody>{list.map((r,i)=>(
              <tr key={i}>
                ...
                <td className="right td-number">₹{parseFloat(r.amount).toLocaleString('en-IN',{minimumFractionDigits:2})}</td>
