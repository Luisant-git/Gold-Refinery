...
export default function PrintReceipt({ voucher, type, onClose }) {
  ...
  const pureTouchVal  = parseFloat(voucher.pure_touch || 99.90);
        ...
        <div style={{ background:'#fff', borderRadius:8, overflow:'hidden', maxHeight:'95vh', overflowY:'auto', minWidth:380, maxWidth:440 }}>
          ...
          <div className="print-receipt" style={{
                ...
                {isExchange
                  ? items.map((item, i) => (
                    <tr key={i}>
                      ...
                      <td style={{ textAlign:'right', padding:'2px 4px' }}>{parseFloat(item.katcha_wt||0).toFixed(3)}</td>
                      <td style={{ textAlign:'right', padding:'2px 4px' }}>{parseFloat(item.balance_touch||0).toFixed(2)}</td>
                      <td style={{ textAlign:'right', padding:'2px 0', fontWeight:'bold' }}>{parseFloat(item.pure_wt||0).toFixed(3)}</td>
                    </tr>
                  ))
                  : items.map((item, i) => (
                    <tr key={i}>
                      ...
                      <td style={{ textAlign:'right', padding:'2px 4px' }}>{parseFloat(item.katcha_wt||item.gross_wt||0).toFixed(3)}</td>
                      <td style={{ textAlign:'right', padding:'2px 4px' }}>{parseFloat(item.touch||0).toFixed(2)}</td>
                      <td style={{ textAlign:'right', padding:'2px 0', fontWeight:'bold' }}>₹{parseFloat(item.amount||0).toFixed(0)}</td>
                    </tr>
                  ))
                }
              </tbody>
            ...
            {isExchange && (
              <div style={{ borderTop:'1px dashed #000', paddingTop:4, marginBottom:6 }}>
                ...
                <div style={{ marginTop:6, marginBottom:4 }}>
                  ...
                  <div style={{ ...row }}>
                    <span>{actualPureWt.toFixed(3)} × {pureTouchVal.toFixed(2)}</span>
                ...
                <div style={{ marginTop:6, fontSize:10 }}>
                  <div>PURE TOKEN No. {items[0]?.token_no || voucher.voucher_no} &nbsp;&nbsp; TOUCH : {pureTouchVal.toFixed(2)}</div>
            ...
            {(isSales || isPurchase) && (
              <div style={{ borderTop:'1px dashed #000', paddingTop:4, marginBottom:6 }}>
                <div style={{ ...row }}>
                  ...
                  <span style={{ fontWeight:'bold' }}>{totalWt.toFixed(3)} g</span>
                </div>
                <div style={{ ...row }}>
                  <span>GROSS AMOUNT</span>
                  <span style={{ fontWeight:'bold' }}>₹{grossAmt.toLocaleString('en-IN', { minimumFractionDigits:2 })}</span>
                </div>
                {deductions > 0 && (
                  <div style={{ ...row }}>
                ...
                <div style={{ borderTop:'1px solid #000', paddingTop:3, marginTop:4, ...row }}>
                  <span style={{ fontWeight:'bold' }}>{isSales ? 'NET PAYABLE TO CUSTOMER' : 'AMOUNT TO PAY'}</span>
                  <span style={{ fontWeight:'bold', fontSize:13 }}>₹{netAmt.toLocaleString('en-IN', { minimumFractionDigits:2 })}</span>
                </div>
                {rate > 0 && (
            ...
            <div style={{ marginTop:28, marginBottom:6 }}>
              <div style={{ borderBottom:'1px solid #000', width:100, marginBottom:4 }}></div>
              <div style={{ fontSize:9 }}>Authorised Signature</div>
            </div>
